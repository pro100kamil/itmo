...
public class RequestHandler {
    ...
    public GetCollectionResponse handleGetCollectionRequest(GetCollectionRequest request) {
        return new GetCollectionResponse(commandManager.getCollectionManager().getLinkedList());
    }

    public GetUsersResponse handleGetUsersRequest(GetUsersRequest request) {
        try {
            return new GetUsersResponse(databaseManager.loadUsers());
        } catch (SQLException e) {
            return new GetUsersResponse(new ArrayList<>());
        }
//        return new GetCollectionResponse(commandManager.getCollectionManager().getLinkedList());
    }
...
//            commandManager.getCollectionManager().sortByName();  //сортируем коллекцию по имени
    ...
    public Response handleRequest(Request request) {
        ...
        else if (request instanceof GetCollectionRequest) {
            return handleGetCollectionRequest((GetCollectionRequest) request);
        }
        else if (request instanceof GetUsersRequest) {
            return handleGetUsersRequest((GetUsersRequest) request);
        }
